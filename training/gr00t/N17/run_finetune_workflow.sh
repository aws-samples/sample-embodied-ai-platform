...
echo "WANDB_DIR: ${WANDB_DIR:-<defaults to OUTPUT_DIR>}"
echo "WANDB_BASE_URL: ${WANDB_BASE_URL:-<not set, offline mode>}"
echo ""
...
# Default to offline W&B logging (logs to WANDB_DIR, sync later)
export WANDB_MODE=${WANDB_MODE:-offline}
# WANDB_DIR is set after OUTPUT_DIR is finalized (see below)
...
if [ "$REPORT_TO" = "wandb" ] && [ "$WANDB_MODE" != "offline" ]; then
    if [ -z "$WANDB_API_KEY" ]; then
        ...
        echo "  Set WANDB_API_KEY, or set REPORT_TO=none to disable logging."
    fi
fi
...
# Write wandb run data inside the job output dir so it persists on EFS
