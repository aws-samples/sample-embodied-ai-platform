...
**Eval mode** (`--context mode=eval`) drops the training learner pod and uses a `(1 + num_rollout_workers)`-pod topology on `<rollout-instance>` (1× L40S each). Set `num_rollout_workers` explicitly so `total_num_envs` is divisible by `1 + num_rollout_workers`: a bare eval uses the **app default `num_rollout_workers=4`** (5 pods), which is **not** valid at the yaml-default `total_num_envs=64` (64 ÷ 5) and crash-loops — use `num_rollout_workers=1` + `eval_total_envs=8` for a smoke (2 pods) or `num_rollout_workers=7` at `total_num_envs=64` for the benchmark (8 pods, 8 envs/GPU). No p5.48xlarge / no Capacity Block required. The head pod runs `entrypoint-eks.sh → eval_embodied_agent.py`; pass `--context save_video=true` to write MP4 rollouts to `${LOG_DIR}/video/eval/` (off by default). See the [Modes](#modes) section above for both smoke and benchmark invocations.
